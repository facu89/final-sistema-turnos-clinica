...
import { sendTurnoPendientePagoPorConvenioNotification } from "../email-resend-convenio";

export async function notificarCambioEstadoTurnoPorConvenio({
  idTurno,
  descripcion,
  nuevoEstado,
  fechaHoraTurno,
  ...
  try {
    ...
    const notificationResult = await sendTurnoPendientePagoPorConvenioNotification({
