...
export async function calcularReintegro(cod_turno: number): Promise<number> {
  console.log("Calculando reintegro para el turno:", cod_turno);
  try {
    const turnoRes = await fetch(
      `/api/turnos/turno-codigo?cod_turno=${cod_turno}`,
    );
    console.log("Respuesta del API de turno:", turnoRes);
    ...
    console.log("Datos del turno para reintegro:", {
      turnoPagado,
      fechaTurno,
      legajoMedico,
    });
    if (turnoPagado && fechaTurno && legajoMedico) {
      console.log("El turno es pagado, procediendo a calcular reintegro.");
