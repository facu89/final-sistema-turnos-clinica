...
import { sendAvisoReasignacion } from "../email-resend-turno-reasignado";
...
export async function reasignarTurnos(turnosAfectados: any[], agenda: any) {
  ...
  for (const turno of turnosAfectados) {
    ...
    if (horario) {
      ...
      console.log("Turno reasignado a:", horario.toISOString());
      //logica para notificar al paciente sobre la reasignacion del turno podria ir aca

      sendAvisoReasignacion({
  ...
  function generarHorariosDisponibles(
    ...
    for (
      ...
      d <= new Date(fecha_Fin.getTime() + 24 * 60 * 60 * 1000 - 1);
      // incluye último día
